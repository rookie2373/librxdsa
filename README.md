...
```bash
sudo apt update
sudo apt install ./librxdsa-0.0.18-amd64.deb
```

### 🎩 Fedora & RedHat (DNF)
Download the `.rpm` asset from our [Releases page](https://github.com) and install natively:
```bash
sudo dnf install ./librxdsa-0.0.18-1.x86_64.rpm
```

### 🪟 Windows (MinGW-w64)
Download `librxdsa-0.0.18.zip` from our Releases page and link the static library inside your workspace path:
```bash
# Include Paths
-Iinclude/rxdsa
